...
Requirements:
    ...
    5. create final tasks dynamically using XCOM values
'''
...
def adv_task_flow_api_list_mapping_dynamic2():
    ...
    def branch_func(minute, ti):
        if not int(minute) % 2:
            print(minute)
            min_last_digit = str(int(minute) % 10)

            # XCOM value pushed steadily
            ti.xcom_push(key="min_last_digit", value=min_last_digit)
            return "even_min"
        else:
            print(minute)
            return "odd_min"
    ...
    def dynamic_task_gen(min, minute, activity, ti):

        # XCOM value not getting pulled steadily.
        min_last_digit = ti.xcom_pull(key="min_last_digit", task_ids="check_minute_of_dag_run")
        print(min_last_digit)
    ...
    # TEST 3: (Trying to use a XCOM pulled value to generate Task dynamically)
    # Result: XCOM is getting pushed fine, but it's not getting pulled steadily.
    # Another issue is that XCOM is a state generated in RUNTIME, while DAG Structure is something determined in  PARSE TIME.
    # Workaround: generate filtered tasks dynamically, and then select only one task based on a condition. Raise SkipException for the rest.

# the dag function must be invoked
dag = adv_task_flow_api_list_mapping_dynamic2()